...
namespace aircraft {
  ...
  bool Aircraft::transitionToState(network::StateId targetState, TransitionSource source) {
    ...
    while (continue_transitioning) {
      ...
      if (!isAllowedTransition(*currentState, requested_target)) {
  ...
  bool Aircraft::sendDiagnosticData() {
    ...
    const auto packet = network::serializePacket(network::PacketType::DIAGNOSTIC_DATA,
