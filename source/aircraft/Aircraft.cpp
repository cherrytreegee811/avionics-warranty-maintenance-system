    ...
    : m_currentState("STANDBY") 
{
    ...
    // Sample fault codes
    m_faultCodes.push_back({101, "Engine temperature sensor fault", std::chrono::system_clock::now()});
...
std::string Aircraft::getCurrentState() const {
...
MaintenanceInfo Aircraft::getLastMaintenance() const {
    return m_lastMaintenance;
}

std::vector<FaultCode> Aircraft::getFaultCodes() const {
    return m_faultCodes;
}

WarrantyInfo Aircraft::getWarranty() const {
    return m_warranty;
}

void Aircraft::setLastMaintenance(const MaintenanceInfo& info) {
    m_lastMaintenance = info;
}
...
void Aircraft::setWarranty(const WarrantyInfo& info) {
    m_warranty = info;
}