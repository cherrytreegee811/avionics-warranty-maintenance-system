      ...
      if (!verified_) {
        ...
        switch (header.type) {
          ...
          case network::PacketType::CLEAR_DIAGNOSTIC_CODE: {
            auto send_confirmation
                = [this](int32_t code, network::DiagnosticCodeClearStatus status) {
                    const bool can_send = connection_
                                          && connection_->getState()
                                                 == network::ConnectionState::VERIFIED;
                    if (can_send) {
