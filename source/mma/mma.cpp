      ...
      try {
        ...
        if (ok) {
          ...
          if (!bytes_to_write.empty()) {
            const bool write_ok = static_cast<bool>(
                file.write(bytes_to_write.data(),
                           static_cast<std::streamsize>(bytes_to_write.size())));
            (void)write_ok;
          }
          if (!file) {
        ...
        if (ok) {
          ...
          if (ok) {
            ...
            if (saved_size > 0U) {
              ...
              const bool read_ok
                  = static_cast<bool>(verify_file.read(read_buffer.data(),
        ...
        case network::PacketType::STATE_CHANGE_CONFIRMATION: {
          if (payload.size() != sizeof(network::StateChangeConfirmation)) {
            ...
            spdlog::info("State change confirmation received from aircraft {}: state change to {}",
                         aircraft_id,
                         network::stateIdToString(confirmation.applied_state));
          ...
          if (!network::deserializeImageChunk(payload, chunk_header, chunk_data)) {
            ...
            if (buffer_it == aircraft_buffers.end()) {
              aircraft_buffers.emplace(chunk_header.image_id,
                                       network::ImageBuffer(chunk_header.image_id,
                                                            chunk_header.format,
              ...
              if (buffer_it->second.addChunk(chunk_header.chunk_index, chunk_data)) {
                ...
                if (!buffer_it->second.validateReassembledCrc(complete_image)) {
                  spdlog::error(
                      ...
                      buffer_it->second.expected_image_crc32,
